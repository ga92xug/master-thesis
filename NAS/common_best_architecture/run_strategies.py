...
strategies_df = pd.read_csv('NAS/data/common_best_architecture/pure_strategies.csv', index_col=0)
...
for strategy_row in strategies_df.iterrows():
    ...
    for dataset_row in strategies_df.iterrows():
...
def get_adjusted_dict(
    ...
    replacement_group_strategy = "strategy_" + map_strategies_2_replacement_groups[dataset_name]

    replacement_group_strategy_row = strategies_df.loc[[replacement_group_strategy]]
    replacement_group_strategy_dict = replacement_group_strategy_row.to_dict()
...
def strategies_on_datasets(
    ...
    for dataset in datasets:
        ...
        for strategy_row in strategies_df.iterrows():
            ...
            if adjust_group and strategy_name != dataset:
                get_adjusted_dict(dataset, strategies_df, strategy_dict)
...
def main():
    ...
    strategies_on_datasets(datasets, strategies_df, adjust_group, choice_2_range_params)
