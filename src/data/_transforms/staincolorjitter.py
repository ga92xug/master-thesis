...
class StainColorJitterWrapper(object):
    def __init__(self, sigma=0.05, p=1.0):
        self.stain_jitter = StainColorJitter(sigma)
        self.p = p
        self.to_tensor = transforms.ToTensor()
        self.to_pil = transforms.ToPILImage()

    def __call__(self, img):
        # Check if we apply the transformation
        if torch.rand(1).item() < self.p:
            img_tensor = self.to_tensor(img)  # Convert image to tensor
            img_tensor = self.stain_jitter(img_tensor)  # Apply StainColorJitter
            img = self.to_pil(img_tensor)  # Convert back to PIL image
        return img

# Example of using the wrapper class
#transform = StainColorJitterWrapper(sigma=sigma, p=p)
