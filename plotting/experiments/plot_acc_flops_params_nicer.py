...
def plot_combined_acc_flops_params(
    ...
    if split_labels_on is None:
        ...
        for i, unique_label in enumerate(unique_labels):
            indices = [i for i, label in enumerate(labels) if unique_label in transform_label_function(label)]
...
if __name__ == "__main__":
    ...
    models = {
        "Gessert et al. single model": {"acc": 68.8, "param_count": 40.76e6, "flops": 19.97e9},
        "EfficientNet pre-trained": {"acc": 62.97, "param_count": 4.01e6, "flops": 0.14e9},
        "EfficientNet": {"acc": 39.76, "param_count": 4.01e6, "flops": 0.14e9},
        "ViT pre-trained": {"acc": 66.34, "param_count": 85.69e6, "flops": 5.5e9},
        "ViT": {"acc": 43.51, "param_count": 85.69e6, "flops": 5.5e9},
        "Eq-NASNet": {"acc": 69.69, "param_count": 0.66e6, "flops": 1.7e9}
    }
    ...
    fig = plot_combined_acc_flops_params(
        models, 
        metric_name="Validation Accuracy Weighted [%]",
        bubble_sizes=[1, 2, 4, 8, 16, 32, 64],
        #split_labels_on="s.split('_pre')[0]",    
    )
