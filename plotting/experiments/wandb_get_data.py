...
from plotting.experiments.wandb_utils import get_flops_per_image, get_parameter_count
...
def download_run(
    ...
    try:
        ...
        print(f"Run {run.id} does not have metric {metric}!")
        quit()
    result["param_count"] = get_parameter_count(run)
    result["flops"] = get_flops_per_image(run) 
