from equivariant.nn import GSpace2D, FieldType, GroupTensor
...
class Swish(EquivariantModule):
    ...
    def forward(self, input: GroupTensor) -> GroupTensor:
        ...
        Applies Swish function on the input fields

        Args:
            input (GroupTensor): the input feature map

        Returns:
            the resulting feature map after Swish has been applied
        ...
        return GroupTensor(
            F.silu(input.tensor, inplace=self._inplace), self.out_type, input.coords
        )
