...
class _RdConv(EquivariantModule, ABC):
    def __init__(
        ...
        self.register_buffer("filter", torch.zeros(*filter_size), persistent=False)
    ...
    def expand_parameters(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        print("self.weights", hash(self.weights.data))
        ...
        print("weights", hash(weights.data))

        self.filter = self.basisexpansion(weights)
        print("filter", hash(self.filter.data))
