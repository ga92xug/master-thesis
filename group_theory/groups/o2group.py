...
import group_theory
from group_theory.utils import psichi, cycle_isclose
    ...
    def _combine_subgroups(self, sg_id1, sg_id2):
        ...
        if sg_id1[0] is None:
            ...
            flip = (
                sg_id1[0]
                + inclusion(
                    sg1.element((0, sg_id2[0] * (2 * np.pi) / sg1.rotation_order))
                ).value[1]
            )
...
# SO2 (and Cyclic) ###############################
def o2_to_so2(so2: Union[SO2, CyclicGroup]):
...
# Flip wrt an axis ######################################
def o2_to_flip(axis: float, flip: CyclicGroup):
...
# Dihedral Group ######################################
def o2_to_dn(axis: float, dn: DihedralGroup):
