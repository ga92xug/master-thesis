...
def eq_drop_connect(inputs, p, training):
    """Drop connect.
    Args:
        input (tensor: BCWH): Input of this structure.
        p (float: 0.0~1.0): Probability of drop connection.
    ...
    batch_size = inputs.tensor.shape[0]
    keep_prob = 1 - p

    # generate binary_tensor mask according to probability (p for 0, 1-p for 1)
    random_tensor = keep_prob
    random_tensor += torch.rand([batch_size, 1, 1, 1], dtype=inputs.tensor.dtype, device=inputs.tensor.device)
    binary_tensor = torch.floor(random_tensor)

    # output = inputs / keep_prob * binary_tensor
    inputs.tensor /= keep_prob
    inputs.tensor *= binary_tensor
    return inputs
...
class Conv2dSamePadding(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, image_size=None, dilation=1, bias=True, groups=1):
        super().__init__()
        ...
        self.conv2d = nn.Conv2d(in_channels, out_channels, kernel_size, stride=self.stride, padding=0, dilation=self.dilation, bias=bias, groups=groups)
