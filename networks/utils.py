...
def conv7x7(in_type: FieldType, out_type: FieldType, stride=1, padding=3, dilation=1, bias=False, sigma=None, F=1., initialize=True):
    """7x7 convolution with padding"""
    fco = _get_fco(F)
    return R2Conv(in_type, out_type, 7,
                     stride=stride,
                     padding=padding,
                     dilation=dilation,
                     bias=bias,
...
def conv5x5(in_type: FieldType, out_type: FieldType, stride=1, padding=2, dilation=1, bias=False, sigma=None, F=1., initialize=True):
    """5x5 convolution with padding"""
    fco = _get_fco(F)
    return R2Conv(in_type, out_type, 5,
                     stride=stride,
                     padding=padding,
                     dilation=dilation,
                     bias=bias,
...
def conv3x3(in_type: FieldType, out_type: FieldType, padding=1, stride=1, dilation=1, bias=False, sigma=None, F=1., initialize=True):
...
def conv1x1(in_type: FieldType, out_type: FieldType, padding=0, stride=1, dilation=1, bias=False, sigma=None, F=1., initialize=True):
    """1x1 convolution"""
    fco = _get_fco(F)
...
def regular_fiber(gspace, planes: int, fixparams: bool = True):
...
"""
def quotient_fiber(gspace, planes: int, fixparams: bool = True):
    ...
    if fixparams:
        planes *= math.sqrt(N * CHANNELS_CONSTANT)
    planes = int(planes)
    return FieldType(gspace, rs * planes).sorted()
"""

def trivial_fiber(gspace, planes: int, fixparams: bool = True):
