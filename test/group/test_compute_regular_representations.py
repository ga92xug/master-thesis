...
import sys
sys.path.append("../../scaling-laws-ecnn")

from group_theory.groups import CyclicGroup
from group_theory.groups import DihedralGroup
...
class TestComputeRegularRepresentations(TestCase):
    ...
    def dihedral_rr_eval(self, dg, rr):
        ...
        # print('Change of Basis')
        print("rr.change_of_basis", rr.change_of_basis)
        # print('Change of Basis Inv')
        # print(rr.change_of_basis_inv)
        # print('RR')
        # n = dg.rotation_order
        ...
        for i, element in enumerate(dg.elements):
            ...
            # print("element", element.value, type(element))
            f = -1 if int(element.value[0]) else 1
            # r = int(np.round(element[1] * dg.rotation_order / (2 * np.pi)))
            r = int(element.value[1])
        
            for k in range(dg.rotation_order):
                rho_i[dg.rotation_order * int(element.value[0]) + ((r + k * f) % dg.rotation_order), k] = 1.0
            for k in range(dg.rotation_order):
                rho_i[dg.rotation_order * (1 - int(element.value[0])) + ((r + k * f) % dg.rotation_order), dg.rotation_order + k] = 1.0
...
if __name__ == '__main__':
    test = TestComputeRegularRepresentations()
    test.test_dihedral_rr_odd()
    #unittest.main()
